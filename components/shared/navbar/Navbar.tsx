    ...
    <div ref={navRef} className="fixed top-0 left-0 right-0 z-50">
      <AnimatePresence>
        <motion.nav
          ...
          className={`relative z-50 flex items-center justify-between nav-spacing backdrop-blur-md bg-black/20 border-b border-white/10 transition-all duration-300 ${
            openDropdown ? "bg-black/40 border-b-transparent" : "bg-black/20"
          }`}
        ...
        >
          ...
          {/* Nav Links - Desktop */}
          <ul className="hidden lg:flex items-center gap-10 font-work-sans">
            {navLinks.map((link) => (
              <li key={link.name}>
                {link.hasDropdown ? (
                  <AnimatedLink 
